...
this.patientService.registerPatients(formData).subscribe(
  (response) => {
    ...
    // this.patientService.sendEmailOnRegistration(response.patientId);
    this.patientService.sendEmailOnRegistration(response.patientId).subscribe(
      () => {
    ...
    this.registerPatientFormsGroup.reset();
